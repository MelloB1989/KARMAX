...
//
// last_message_id is assigned BEFORE last_seen_at because MySQL evaluates an
// upsert's assignments left to right against the row as it is being written:
// updating last_seen_at first would falsify the comparison the id depends on.
// SQLite and Postgres read the pre-update row and do not care about the order.
